...
class Rollout(object):
    ...
    def __init__(self, e_model, g_model, update_rate):
    ...
    def get_reward(self, x, num, discriminator):
        ...
        for i in range(num):
            for l in range(1, seq_len):
                data = x[:, 0:l]
                samples = self.g_own_model.prob2pred(self.g_own_model.sample(self.e_own_model(one_hot(data, self.g_own_model.vocab_size))))
    ...
    def update_params(self):
        ...
        for name, param in self.g_ori_model.named_parameters():
            dic[name] = param.data
        for name, param in self.g_own_model.named_parameters():
            if name.startswith('emb'):
                param.data = dic[name]
            else:
                param.data = self.update_rate * param.data + (1 - self.update_rate) * dic[name]